...
export default function ProductPage(props) {
  ...
  function show() {
    return props.products.map((pro) => {
      if (pro.id == s.id) {
        return (
          <div className="d-card" key={pro.id} id={pro.id}>
            ...
            <div className="pricing">
              <div className="d-old-price">
                Old Price: <span>{pro.oldPrice}</span>
              </div>
            ...
            {pro.isAdded ? (
              <>
                <button id={pro.id} onClick={props.toggle}>
                  Remove From The Card
                </button>
                <input
                  onChange={props.onChange}
                  defaultValue={
                    pro.totalPrice ? pro.totalPrice / pro.newPrice : 1
                  }
                  min={1}
                  type="number"
                  name="count"
                ...
                />
                <div className="total">
                  {pro.totalPrice ? pro.totalPrice : pro.newPrice}
                </div>
              </>
            ) : pro.isAvailable ? (
              <button id={pro.id} onClick={props.toggle}>
